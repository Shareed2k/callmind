...
async fn run_serve(config_path: Option<PathBuf>) -> Result<()> {
    ...
    // The transcription stack is assembled by the library, not here, so a remote
    // worker on another machine builds the identical one.
    let callmind_transcript::BuiltTranscriber {
        transcriber,
        stt_engines,
    } = callmind_transcript::build_transcriber(&config.models)?;
